...
const mapDispatchToProps = (dispatch, ownProps) => {
  ...
  return {
    action: () => {
      let id;
      let id2 = null;
      let messageInfo = "IPFS' connection may take a while, please wait until you are logged ";
      if(store.getState().logged === false){
        //logged out
        toastManager.add(messageInfo+"in.", INFOTOAST, (x)=>{id=x});
        toastManager.add(didYouKnowThat(), DIDYOUKNOWTOAST, (x)=>{id2=x})
      }else{
        //logged in
        toastManager.add(messageInfo+"out.", INFOTOAST, (x)=>{id=x});
      }
